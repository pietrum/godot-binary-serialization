...
describe('misc types', () => {
  beforeAll(() => {
    ...
    jest.mock('../decode/node_path', () => jest.fn(() => 'node_path'));
    jest.mock('../decode/array', () => jest.fn(() => 'array'));
  });
  ...
  test('should resolve decoder as misc type NodePath', () => {
    // given
    const variant = Buffer.from('0f000000', 'hex');

    // when
    const decoder = DECODE(variant);

    // then
    expect(decoder).toBe('node_path');
  });

  test('should resolve decoder as misc type Array', () => {
    // given
    const variant = Buffer.from('13000000', 'hex');

    // when
    const decoder = DECODE(variant);

    // then
    expect(decoder).toBe('array');
  });
});
